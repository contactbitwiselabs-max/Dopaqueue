 ...
 */
function setStoredTheme(theme: ThemeType) {
 ...
 * Set theme
 */
export function setThemeGlobal(theme: ThemeType) {
...
export function useTheme() {
  const [theme, setThemeState] = useState<ThemeType>(() => getCurrentTheme());
...
export function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="relative flex items-center justify-center p-2 w-9 h-9 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors overflow-hidden"
      ...
      <AnimatePresence mode="wait" initial={false}>
        {isDark ? (
          <motion.svg 
            ...
            className="w-5 h-5 text-yellow-500 absolute" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
          ...
          <motion.svg 
            ...
            exit={{ y: -20, opacity: 0, rotate: 90 }}
            transition={{ duration: 0.2 }}
            className="w-5 h-5 text-zinc-600 dark:text-zinc-400 absolute" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
