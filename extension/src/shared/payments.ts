...
export function getPaymentProvider(): string {
  const config = getValidatedConfig() as any;
 ...
 * @param plan - Plan ID (PRO_MONTHLY, PRO_ANNUAL, LIFETIME)
 * @param options - Checkout options
 */
export async function createCheckoutSession(plan: string, options: CheckoutOptions = {}): Promise<any> {
  const provider = getPaymentProvider();
 ...
 */
async function createLemonSqueezyCheckout(plan: string, options: CheckoutOptions = {}) {
  const config = getValidatedConfig() as any;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error((error as any).message || `Lemon Squeezy API error: ${response.status}`);
    }
 ...
 */
async function createStripeCheckout(plan: string, options: CheckoutOptions = {}) {
  const config = getValidatedConfig() as any;
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error((error as any).message || `Stripe API error: ${response.status}`);
    }
 ...
 */
async function createTestCheckoutSession(plan: string, options: CheckoutOptions = {}) {
 ...
 */
export async function verifyLicenseKey(licenseKey: string): Promise<any> {
  const provider = getPaymentProvider();
 ...
 */
async function verifyLemonSqueezyLicense(licenseKey: string) {
  const config = getValidatedConfig() as any;
  const apiKey = config.LEMON_SQUEEZY_API_KEY;
 ...
 */
async function verifyStripeLicense(licenseKey: string) {
 ...
 */
async function verifyTestLicenseKey(licenseKey: string) {
  // In test mode, accept specific test keys
  const testKeys: Record<string, any> = {
    'TEST-PRO-MONTHLY': {
      valid: true,
      tier: 'pro_monthly',
